...
struct NoKV_data_s {
	...
	byte ConsoleCertificate[0x1A8];
	byte ConsoleObfuscationKey[0x10];
	byte ConsolePrivateKey[0x1D0];
	byte XeIkaPrivateKey[0x390];

	byte XeIkaCertificateInquiryData[0x24];
	byte ConsoleSerialNumber[0xC];
	XE_CONSOLE_ID consoleID;

	WORD XamRegion;
	WORD XamOdd;
	QWORD restrictedPrivilegesFlags;
...
namespace KV{
	...
	public:
		...
		detour<NTSTATUS> DetourKerbAddConsoleCertHashPrePreAuth;
		detour<NTSTATUS> DetourXeKeysGetConsoleCertificate;
		detour<void> DetourFormatXenonConsoleCertificatePrincipalName;
		detour<NTSTATUS> DetourGetSerialNumber;

		HRESULT ini(NET::CLIENT &nclient);
