...
void PatchInJump(DWORD* addr, DWORD dest, BOOL linked = false, bool run = false);
...
public:
	...
	virtual BOOL setupDetour(DWORD dwModuleBaseAddr, char* szImportedModuleName, int Ordinal, PVOID Destination)
	{
		...
		HANDLE hModule;
		Native::XexPcToFileHeader((PVOID)dwModuleBaseAddr, (PLDR_DATA_TABLE_ENTRY*)&hModule);
		return setupDetour(hModule, szImportedModuleName, Ordinal, Destination);
	}

	///Will hook the import of a module
	virtual BOOL setupDetour(HANDLE hModule, const char* szImportedModuleName, int Ordinal, PVOID Destination)
	{
		DWORD dwAddress;
		HANDLE hImportedModule;
		if (Native::XexGetModuleHandle(const_cast<char*>(szImportedModuleName), &hImportedModule) == ERROR_SUCCESS) {
	...
	///Will hook the import of a module
	virtual BOOL setupDetour(const char* Module, const char* szImportedModuleName, int Ordinal, PVOID Destination)
	{
		HANDLE mHandle;
		if (Native::XexGetModuleHandle(const_cast<char*>(Module), &mHandle) == ERROR_SUCCESS) {
			return setupDetour(mHandle, szImportedModuleName, Ordinal, Destination);
		}
		return FALSE;
	}

	///Will hook the export of a module
	virtual BOOL setupDetour(HANDLE hModule, DWORD Ordinal, PVOID Destination)
	{
		DWORD dwAddress;

		if (hModule == NULL) return FALSE;

		dwAddress = Native::getProcAddress(hModule, Ordinal);

		if (dwAddress == NULL) return FALSE;

		return setupDetour(dwAddress, Destination);
	}

	///Will hook the export of a module
	virtual BOOL setupDetour(const char* szModule, int Ordinal, PVOID Destination)
	{
		DWORD dwAddress;
		HANDLE mHandle;
		if (Native::XexGetModuleHandle(const_cast<char*>(szModule), &mHandle) == ERROR_SUCCESS) {
			dwAddress = Native::getProcAddress(mHandle, Ordinal);
	...
	virtual BOOL setupDetour(DWORD Address, PVOID Destination)
	{
		...
		if (Addr != Address || SaveStub == 0) {
			...
			Mem::Copy(OriginalAsm, (PVOID)Address, 0x10);
	...
	void takeDownDetour() {
		if (Addr && MmIsAddressValid((PVOID)Addr))
			Mem::Copy((PVOID)Addr, OriginalAsm, 0x10);
