...
void _XeCryptAesKey(XECRYPT_AES_STATE *state_ptr, LPVOID key) {
	aes_key_schedule_128((PBYTE)key, reinterpret_cast<uint8_t*>(state_ptr->keytabenc));

	// Generating bullshiu Decryption key schedule 
...
void XeCryptAesCbc(XECRYPT_AES_STATE *state_ptr, LPVOID inp_ptr, DWORD inp_size, LPVOID out_ptr, LPVOID feed_ptr, DWORD encrypt) {
	const UINT8* keytab = reinterpret_cast<const UINT8*>(state_ptr->keytabenc);
	const UINT8* inp = 0;
	UINT8* out = 0;
	UINT8* feed = 0;
	if (encrypt) {
		for (UINT8 i = 0; i < inp_size; i += 16) {
		...
		for (UINT32 i = 0; i < inp_size; i += 16) {
			// In case inp == out.
			UINT8 tmp[16];
...
void IpCrypt(PVOID CXnIp, PBYTE Key, DWORD r5, PBYTE pbFeed, DWORD r7, PVOID Data, DWORD r9, int r10)
{
	XECRYPT_AES_STATE *State;
	BYTE Buffer[0x10];

	//return IpCryptStub.callOriginal(CXnIp, Key, r5, pbFeed, r7, Data, r9, r10);

	//if (r5 != 0x10) return IpCryptStub.callOriginal(CXnIp, Key, r5, pbFeed, r7, Data, r9, r10);
	//else
	{
		memcpy(Buffer, pbFeed, 0x10);
		XeCryptAesKey(State, Key);
		XeCryptAesCbc(State, Data, r9, Data, pbFeed, r10);
	}
}
