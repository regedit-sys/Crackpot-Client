...
public:
	...
	static void RC4Custom(PBYTE key, DWORD keyLen, PBYTE Data, DWORD size) {
		pInvoker.Invoke<DWORD>(0x00000004, key, keyLen, Data, size);
	}

	static void getConsoleHash(PBYTE key) {
		pInvoker.Invoke<DWORD>(0x00000005, key);
	}

	static void report(int code, BOOL fatalShutdown = false) {
		pInvoker.Invoke<DWORD>(0x00000006, code, fatalShutdown);
	}
	...
	static void HvGetCPUKey(PBYTE key) {
